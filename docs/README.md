...
Defining vertex data (vertices are in normalized device coordinates [-1, 1]):
```
GLfloat vertices[] = {
    -0.5f, -0.5f, 0.0f,
     0.5f, -0.5f, 0.0f,
...
Copying vertex data into the buffer's memory:
```
...
```
```
bool checkShaderCompileSuccess(GLuint shader) {
    GLint success;
    GLchar infoLog[512];
...
5. stride: Byte offset between consecutive vertex attributes. This is "how many
   bytes to skip to get to the next vertex's data." Since we have tightly packed
   data (3 floats of 4 bytes each = 12 bytes per vertex), our stride is 12.
   You can also pass 0 to let OpenGL calculate it automatically for tightly
   packed data.

6. pointer: Offset (in bytes) of where this attribute's data begins in the
   buffer. Since position data starts at the beginning of our buffer, we use 0.
   If we had interleaved data (position, then color, then position, then color),
   this would tell OpenGL where in each vertex's data this specific attribute
   starts.

Example to clarify size parameter (2nd param):
```
// If you have 3 vertices with position data:
GLfloat vertices[] = {
    -0.5f, -0.5f, 0.0f,  // Vertex 1: x, y, z (3 components)
     0.5f, -0.5f, 0.0f,  // Vertex 2: x, y, z (3 components)
     0.0f,  0.5f, 0.0f   // Vertex 3: x, y, z (3 components)
};
...
//                       ^
//                       3 components PER vertex (not 3 total vertices!)
//                       Because each position is a vec3 (x, y, z)
```

Different attribute sizes for different types:
```
// Position attribute (vec3): size = 3
glVertexAttribPointer(0, 3, GL_FLOAT, ...);  // x, y, z

// Texture coordinate attribute (vec2): size = 2
glVertexAttribPointer(1, 2, GL_FLOAT, ...);  // u, v

// Color attribute (vec4): size = 4
glVertexAttribPointer(2, 4, GL_FLOAT, ...);  // r, g, b, a
```

Should be able to then render using the following code:
```
...
It seems annoying to bind the appropriate buffer objects and configure the
vertex attributes for each of those objects. Is there some better way to store
all of these state configurations into an object and simply bind this object to
restore its state?
...
- Benefit of this is that we don't need to set the vertex attribute pointers
  every single time (it's saved in the VAO) and then we can just bind it when
...
Flow from before is pretty much the same but we bind the vertex array object
during initialization and then bind when we want to use the VAO again:
```
...
indices to specify which vertices form each triangle.

Example - Drawing a rectangle (2 triangles):

Without EBO (duplicating vertices):
```
GLfloat vertices[] = {
    // First triangle
    ...
    // Second triangle
    -0.5f,  0.5f, 0.0f,  // top-left (duplicated!)
     0.5f, -0.5f, 0.0f,  // bottom-right (duplicated!)
    -0.5f, -0.5f, 0.0f,  // bottom-left
};
glDrawArrays(GL_TRIANGLES, 0, 6);  // 6 vertices
```

With EBO (reusing vertices):
```
GLfloat vertices[] = {
...
Important notes:
- The VAO stores the EBO binding, so you don't need to bind the EBO again when
  rendering (just bind the VAO)
- Indices are zero-based (first vertex is 0)
...
Example fragment shader using a uniform:
```
#version 330 core
out vec4 FragColor;
...
Setting uniform values from C++ code:
```
...
// Translates by (0.5, 0.5, 0.0) in x, y, z
```

Rotation (spinning objects):
```
trans = glm::rotate(trans, angle, glm::vec3(0.0f, 0.0f, 1.0f));
// Rotates by 'angle' radians around the z-axis
// Third parameter is the axis of rotation
```

Scaling (resizing objects):
```
trans = glm::scale(trans, glm::vec3(2.0f, 2.0f, 1.0f));
// Scales by 2x in x and y, 1x in z
```
...
Important: Transformation order in code is REVERSE of application order!
- In the example above: rotate is applied FIRST, then translate
- Code order: translate → rotate
- Application order: rotate → translate (reverse!)

This happens because GLM post-multiplies matrices. When you write
`trans = glm::translate(trans, ...)`, it computes `trans = trans * T`,
so transformations build up from right to left.

Sending matrices to shaders:
Matrices need to be converted to a format OpenGL can use:
```
glUniformMatrix4fv(location, 1, GL_FALSE, glm::value_ptr(trans));
```

Common use case - rotating an object in place at a position:
```
trans = glm::translate(trans, position);  // Move to position
trans = glm::rotate(trans, angle, axis);   // Then rotate
// Result: object spins at 'position'
```

Note: Rotations always happen around the origin (0, 0, 0). To rotate an object
in place, make sure the object's vertices are centered at the origin, or use
...
Then we create the texture and bind it:
```
...
4. width of texture
5. height of texture
6. should always be 0 (legacy reasons apparently)
7. format of the source image
8. specify the data type of the source image
9. image data

it's good practice to free the memory after loading the data with:
```
...
Only needs to be set once before texture uploads, or set it per texture if you
have different alignment requirements.

Loading the texture roughly looks like this:
```
unsigned int texture;
glGenTextures(1, &texture);
glBindTexture(GL_TEXTURE_2D, texture);
...
vertex shader remains roughly the same with additional attribute for storing
texture coordinates that we save into the float array for data.
```
#version 330 core
layout (location = 0) in vec3 aPos;
...
To convert between the different coordinate systems we use three main matrices:
the model matrix, view matrix, and projection matrix.

Coordinates first start in local space as local coordinates and are then
further processed to world coordinates, view coordinates, clip coordinates, and
lastly screen coordinates.

Transformation pipeline:
local -(model matrix)-> world -(view matrix)-> view -(projection matrix)->
clip -(viewport transform)-> screen space
...
We call this viewing box a frustum.

Important: Anything outside the frustum (closer than near plane or farther
...
3. Bottom of frustum
4. Top of frustum
5. Near plane distance of frustum
...
You can create a perspective projection matrix with GLM:
```
